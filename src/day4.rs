...
fn parse_logs(input: &str) -> Vec<GuardReport> {
    ...
    return input.lines().map(|line| {
        ...
        let timestamp = NaiveDateTime::parse_from_str(caps.name("date").expect("date").as_str(), "%Y-%m-%d %H:%M").expect("dateparse");
        let report_type = caps.name("id")
            .map_or_else(
    ...
    for report in reports {
        match report.report_type {
            ...
            ReportType::WakesUp => {
                ...
                let sleep_times = guard_sleeps.entry(current_guard).or_insert_with(|| vec![0; 60]);
...
pub fn part1(input: &str) -> i32 {
    ...
    let (best_guard, best_minutes) = guard_sleeps.into_iter()
        .max_by_key(|(id, minutes)| { minutes.iter().sum::<i32>() }).unwrap();

    let best_minute = best_minutes.iter().enumerate().max_by_key(|&(_, val)| { val }).unwrap().0;
    println!("guard #{}, minute {}, answer {}", best_guard, best_minute, best_guard * best_minute as u32);
    0
}
...
pub fn part2(input: &str) -> i32 {
    ...
    let (best_guard, best_minutes) = guard_sleeps.into_iter()
        .max_by_key(|(_, minutes)| { *minutes.iter().max().unwrap() }).unwrap();
